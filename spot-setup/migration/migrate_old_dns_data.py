...
def main():
  ...
  util.execute_cmd('mv {0}/data/dns/ {0}/data/backup/'.format(old_oa_path),log)
  ...
  for folder in os.listdir('{0}/ipynb/dns/'.format(new_oa_path)):
    util.execute_cmd('cp {0}/oa/dns/ipynb_templates/Advanced_Mode_master.ipynb {1}/ipynb/dns/{2}/Advanced_Mode.ipynb'.format(old_oa_path, new_oa_path, folder),log)

  log.info("Copying threat investigation ipynb template to each existing day in new Spot location")
  for folder in os.listdir('{0}/ipynb/dns/'.format(new_oa_path)):
    util.execute_cmd('cp {0}/oa/dns/ipynb_templates/Threat_Investigation_master.ipynb {1}/ipynb/dns/{2}/Threat_Investigation.ipynb'.format(old_oa_path, new_oa_path, folder),log)
